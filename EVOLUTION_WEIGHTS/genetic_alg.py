...
class GA:
    def __init__(self, mlp: MLP, train_df, test_df, version: str, population, num_generations: int = 3, tournament_size: int = 3, crossover_probability: float = 0.9, coin_toss: float = 0.5, mutation_probability: float = 0.02, mutate_sigma: float = 0.1, classes = None, n_size= None, verbose: str = None):
        self.mlp = mlp
    ...
    def fitness(self):
        ...
        for i in range(len(self.population)):
            # print(i)
            result = UTILS.get_performance(UTILS, self.population[i], self.classes, self.mlp, self.test_df)
