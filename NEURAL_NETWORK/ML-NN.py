...
class MultiLayer_NeuralNetwork:
    ...
    def feedforward_neural_network(self, row):
        ...
        self.output_matrix[0] = row_output
        ...
        for i, w in enumerate(self.weight_matrix):
            sum = np.dot(row_output, w)
            # get hidden layer output
            zh_output = self.sigmoid(sum)
            self.output_matrix[i+1] = zh_output
    ...
    def back_propagation(self, error):
        ...
        for i in reversed(range(len(self.update_matrix))):
            ...
            self.update_matrix[i] = np.dot(current_outputs, update)
